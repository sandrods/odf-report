...
There are times that your data may be very complex (for instance a book with many chapters).  Use the master document feature of Open Office and create a master document template, and subdocuments. In your 'master' records, you must have a field called subdoc that contains the complete filename of the subdocument. Just use the [SD] tag in the template to specify that a subdocument needs to go there.
...
report.generate("master_document.odt")</pre>

h3. Open Office Settings

If you are using the master document feature, you will need to be able to change the settings in the Open Office master document.  For example, the default setting for 'update links?' is set to 'ask'.  Add the following code to your document so it will not ask this question:

<pre>
  report = ODFReport::Report.new("reports/invoice.odt") do |r|
    report.change_setting( 'LinkUpdateMode', [2,"short"] )
  end
</pre>

You will need to specify the name of the setting, and an array with the value and type of variable.  See the settings.xml file for examples.
